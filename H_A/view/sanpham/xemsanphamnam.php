...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['noidung_binhluan'])) {
    ...
    if ($noidung !== '') {
        ...
        header("Location: xemsanphamnam.php?id=$masp#binhluan");
        exit;
    } else {
            ...
            <div>
                ...
                <div class="category-detail" id="ct<?= $index ?>" style="display:none;">
                    <?php foreach ($loai['chitiet'] as $ct): ?>
                        <div><a href="../sanpham/thoitrangnam.php?machitiet=<?= htmlspecialchars($ct['MaChiTiet']) ?>"><?= htmlspecialchars($ct['TenChiTiet']) ?></a></div>
                    <?php endforeach; ?>
                </div>
            </div>
        <?php endforeach; ?>
