...
// Lấy danh sách loại nam và chi tiết loại
$loai_query = mysqli_query($conn, "SELECT * FROM loainam WHERE GioiTinh = 'nam'");
$ds_loai = [];
...
// Truy vấn sản phẩm
$sql = "SELECT * FROM sanphamnam WHERE GioiTinh = 'nam'";
        ...
        <?php foreach ($ds_loai as $loai): ?>
            <div>
                ...
                <div class="category-detail">
                    <div><a href="thoitrangnam.php?maloai=<?= $loai['MaLoai'] ?>">Tất cả <?= htmlspecialchars($loai['TenLoai']) ?></a></div>
                    <?php foreach ($loai['chitiet'] as $ct): ?>
                        <div><a href="thoitrangnam.php?machitiet=<?= $ct['MaChiTiet'] ?>"><?= htmlspecialchars($ct['TenChiTiet']) ?></a></div>
                    <?php endforeach; ?>
                </div>
            </div>
        <?php endforeach; ?>
    ...
    <div class="filter-options">
        ...
        <div class="product-list">
            <?php while ($row = mysqli_fetch_assoc($sanpham_query)): ?>
                ...
                $giaKM = null;
                if ($km) {
                    // Kiểm tra ngày hôm nay có nằm trong khoảng áp dụng khuyến mãi không
                ...
                <div class="product">
                    <a href="xemsanphamnam.php?id=<?= $masp ?>">
                    ...
                    <?php endif; ?>
                    <br>
                    <a href="xemsanphamnam.php?id=<?= $masp ?>">Xem chi tiết</a>
                </div>
            <?php endwhile; ?>
        </div>
    </div>
